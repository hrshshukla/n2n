...
# N2N VPS Setup Script
# This script helps set up n2n on a fresh Ubuntu/Debian VPS

# Exit on error
set -e

echo "=== N2N VPS Setup Script ==="
echo "This script will install Java, Node.js, Nginx, and set up Node 2 Node ."
...
# Create the n2n configuration file with the correct content
echo "Creating /etc/nginx/sites-available/n2n..."
cat <<EOF | sudo tee /etc/nginx/sites-available/n2n
server {
    listen 80;
...
# Create the symbolic link to enable the n2n site
sudo ln -sf /etc/nginx/sites-available/n2n /etc/nginx/sites-enabled/n2n
...
if [ $? -eq 0 ]; then
    ...
    echo "Nginx configuration test failed. Please check /etc/nginx/nginx.conf and /etc/nginx/sites-available/n2n."
    exit 1
fi
...
CLASSPATH="target/n2n-1.0-SNAPSHOT.jar:$(mvn dependency:build-classpath -DincludeScope=runtime -Dmdep.outputFile=/dev/stdout -q)"
pm2 start --name n2n-backend java -- -cp "$CLASSPATH" n2n.App
